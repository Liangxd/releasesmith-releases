...
在 ReleaseSmith 的 GitHub“自动跨平台发布”模式中填写源码仓、公开发布仓和 `v1.0.0`。确认后应用会准备两个仓库，推送当前已提交源码到目标分支，并从同一个提交推送该标签。标签推送会触发 Actions；已有同名标签会被预检查阻止，不会覆盖。宣传文案、海报和最后的完整资料包导出步骤都可以直接跳过；如果只需要完成 GitHub 发布，之后仍可返回第 9 步生成资料包。
